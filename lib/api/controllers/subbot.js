...
async function getList(req, res) {
    ...
    // Send API response
    res.status(200);
    res.json({ "code": 200, "message": "", "data": subbots, });
...
async function post(req, res) {
    ...
    // Send API response
    res.status(201);
    res.json({ "code": 201, "message": "Created", "data": subbot, });
...
async function del(req, res) {
    ...
    if (
        ...
        res.status(404);
        res.json({ "code": 404, "message": "Required field(s) missing", });
        return;
    }
    ...
    try {
        ...
        if (subbot) {
            let subbotToDelete = JSON.stringify(subbot);
            await subbot.destroy();
            minilogger.print(`Subbot deleted // ` + subbotToDelete);
        }
