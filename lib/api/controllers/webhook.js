...
/**
 * We tell Telegram a bot's webhook url on this API
 *
 * @param {object} [req]
 * @param {object} [res]
 */
async function postWebhookSet(req, res) {

    let functionName = "postWebhookSet";
    ...
    try {
        ...
        let outcome = await telegram.setWebhook(tgBotUsername);
        if (outcome !== true) {
            throw { message: 'There was an error, check the logs' };
        }

        res.status(201);
        res.json({ "code": 201, "message": `Good`, });
 ...
 * POST
 *
 * Executed when Telegram hits our webhook url to send us updates
 *
 * @param {object} [req]
 * @param {object} [res]
 */
