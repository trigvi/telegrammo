...
const OutgoingQueue = sequelize.define('OutgoingQueue', {
    outgoingQueueId:{ type: Sequelize.INTEGER, primaryKey: true, autoIncrement: true, },
...
async function sync() {
    ...
    try {
        ...
        minilogger.print("Db sync successful");
    } catch (err) {
    ...
    try {
        await vacuum();
        minilogger.print("Db vacuum successful");
    } catch (err) {
        minilogger.print(`ERROR // ${moduleName} // ${functionName} // ${err.message}`);
        return false;
    }
